...
class Auth extends CI_Controller {
    ...
    public function login() {
        ...
        //login check if login then redirect to dashboard
        if (loginCheck()) { // global helper function
            redirect('admin/profile/dashboard');
        }
            ...
            if ($this->form_validation->run()) {
                ...
                if (count($result) > 0) {
                    ...
                    if ($this->user_model->updateUser($updateData, $result->user_id)) {
                        ...
                        $comment .= "<p>Thanks<br/>";
                        $comment .= $this->config->item('title') . "</p>";

                        $this->load->library('email');

                        $this->email->from('support@(YOUR SITE NAME).com', $this->config->item('title'));
                        $this->email->to($email);

                        $this->email->subject('Admin Password Reset request received for - ' . $this->config->item('title'));
                        $this->email->message($comment);
                        $this->email->set_mailtype('html');

                        if ($this->email->send()) {
                            echo '<div class="alert alert-success fade in"><button data-dismiss="alert" class="close close-sm" type="button"><i class="icon-remove"></i></button>You have been sent an email with password reset instruction. If the email does not arrive within several minutes, be sure to check your spam or junk mail folders.</div>';
                        } else {
                            echo '<div class="alert alert-block alert-danger fade in"><button data-dismiss="alert" class="close close-sm" type="button"><i class="icon-remove"></i></button>Error: mail not send....</div>';
                        }
                    }
                } else
                    echo '<div class="alert alert-block alert-danger fade in"><button data-dismiss="alert" class="close close-sm" type="button"><i class="icon-remove"></i></button>Password reset Failed!! <br>Please enter valid Email ID to reset your password.</div>';
            }else {
                echo '<div class="alert alert-block alert-danger fade in"><button data-dismiss="alert" class="close close-sm" type="button"><i class="icon-remove"></i></button>' . validation_errors() . '</div>';
            }
        }
    }
